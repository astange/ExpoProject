...
5. Disconnect from the screen by pressing Control+A, then pressing D. Run the site the same way - "screen python /app/website/routes.py", then disconnect from that screen the same way.
6. Navigate to localhost:5001 if you're running the form, localhost:5002 for the website, and have fun. You can use redis-cli to check out the entries the form is creating in the database.
